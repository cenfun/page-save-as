...
const saveAsPdf = async (option) => {
    ...
    Object.assign(pdfOption, option.pdfOption);

    let code = 0;
    await option.page.pdf(pdfOption).catch(function(err) {
        console.log(EC.red(err));
        code = 1;
    });
    return code;
...
module.exports = async (args) => {
    ...
    if (option.edit) {
        if (option.type === "pdf") {
            ...
            console.log(EC.yellow("so it can NOT edit for a pdf"));
        } else {
    ...
    const code = await Util.tasksResolver(tasks, option);
