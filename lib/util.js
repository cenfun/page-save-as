...
const Util = {
    ...
    scroll: async (page) => {
        ...
        const hw = info.windowWidth * 0.5;
        const hh = info.windowHeight * 0.5;
        await page.mouse.move(hw, hh);

        const deltaY = Math.round(info.windowHeight * 0.9);
        const bodyHeight = info.bodyHeight;
        let i = 0;
        while (i < bodyHeight) {
    ...
    launchBrowser: async (browserOption) => {
        ...
        const viewport = {};
        if (Util.option.width) {
            viewport.width = Util.toNum(Util.option.width);
        }

        const defaultViewport = Util.getDefaultViewport(viewport);
